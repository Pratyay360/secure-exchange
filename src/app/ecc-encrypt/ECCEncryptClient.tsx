...
import { Copy, Shield, AlertCircle, CheckCircle2 } from "lucide-react";
...
export default function ECCEncryptClient() {
  ...
  const handleEncrypt = async () => {
    ...
    setIsLoading(true);
    try {
      const encryptedBase64 = await encryptWithECC(message, publicKey);
    ...
    <div className="min-h-screen flex items-center justify-center bg-background py-12 px-4">
      <div className="w-full max-w-2xl">
        <Card className="shadow-2xl border-2 border-green-200 dark:border-green-800">
          <CardHeader className="bg-gradient-success text-white p-8">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-white/20 rounded-xl backdrop-blur-sm">
                <Shield className="w-8 h-8" />
              </div>
              <div>
                <CardTitle className="text-3xl font-bold">ECC Encryption</CardTitle>
                <CardDescription className="text-green-100 mt-2 text-base">
                  Encrypt your message using Elliptic Curve Cryptography
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          ...
          <CardContent className="p-8 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <textarea
                className="w-full h-40 p-4 border-2 border-gray-300 dark:border-gray-600 rounded-xl resize-none focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-800 dark:text-white transition-all"
            ...
            <Button 
              className="w-full h-12 text-lg font-semibold bg-green-600 hover:bg-green-700 transition-all" 
